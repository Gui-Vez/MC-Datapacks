...
execute if score $AttackID VAR matches 0 as @e[type=phantom, limit=1, tag=Water_Boss] at @s if entity @e[tag=WaterBoss_Targeted, limit=1, sort=nearest, distance=0..10] run scoreboard players set $AttackID VAR 1
